...
竖版适合手机竖屏视频，坐标体系与横版不同：

| 方向 | 像素 | x 范围 | y 范围 |
|------|------|--------|--------|
| 横版 landscape | 1280×720 | [-7.1, 7.1] | [-4, 4] |
| 竖版 portrait | 720×1280 | [-4, 4] | [-7.1, 7.1] |

```
web_persistent_start(project="short-video", orientation="portrait")
```

竖版布局建议：图形纵向排列 `arrange(DOWN)`，公式垂直展示，充分利用 y 方向空间。

模板参考 → [docs/templates/portrait_demo.py](templates/portrait_demo.py)
