...
__all__ = ["tex_to_svg_file", "async_tex_to_svg_file", "precompile_tex_expressions"]

_tex_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="manim-tex-")
_async_tex_cache: dict[str, asyncio.Future] = {}
_svg_memory_cache: dict[str, Path] = {}
...
def tex_to_svg_file(
    ...
    if not config["no_latex_cleanup"]:
        delete_nonsvg_files()
    _svg_memory_cache[tex_hash(expression)] = svg_file
    return svg_file


def _tex_cache_key(expression: str, environment: str | None, tex_template: TexTemplate | None) -> str:
    return f"{tex_hash(expression)}:{environment}:{tex_template.tex_compiler if tex_template else 'default'}"


async def async_tex_to_svg_file(
    expression: str,
    environment: str | None = None,
    tex_template: TexTemplate | None = None,
) -> Path:
    r"""Async version of tex_to_svg_file. Uses asyncio subprocess for non-blocking LaTeX compilation."""
    if tex_template is None:
        tex_template = config["tex_template"]

    cache_key = _tex_cache_key(expression, environment, tex_template)

    if cache_key in _svg_memory_cache:
        return _svg_memory_cache[cache_key]

    if cache_key in _async_tex_cache:
        return await _async_tex_cache[cache_key]

    tex_file = generate_tex_file(expression, environment, tex_template)
    svg_file = tex_file.with_suffix(".svg")
    if svg_file.exists():
        _svg_memory_cache[cache_key] = svg_file
        return svg_file

    loop = asyncio.get_event_loop()
    future = loop.run_in_executor(
        _tex_executor,
        _compile_and_convert_sync,
        tex_file,
        tex_template,
    )
    _async_tex_cache[cache_key] = future
    try:
        result = await future
        _svg_memory_cache[cache_key] = result
        return result
    finally:
        _async_tex_cache.pop(cache_key, None)


def _compile_and_convert_sync(tex_file: Path, tex_template: TexTemplate) -> Path:
...
def precompile_tex_expressions(
    expressions: list[str],
    environment: str | None = None,
    tex_template: TexTemplate | None = None,
) -> list[Any]:
    r"""Batch precompile: submit all LaTeX expressions for compilation before play().

    Returns list of Future objects. Call future.result() to get the SVG path.
    """
    if tex_template is None:
        tex_template = config["tex_template"]
    futures = []
    for expr in expressions:
        cache_key = _tex_cache_key(expr, environment, tex_template)
        if cache_key in _svg_memory_cache:
            from concurrent.futures import Future
            f = Future()
            f.set_result(_svg_memory_cache[cache_key])
            futures.append(f)
            continue
        tex_file = generate_tex_file(expr, environment, tex_template)
        svg_file = tex_file.with_suffix(".svg")
        if svg_file.exists():
            _svg_memory_cache[cache_key] = svg_file
            from concurrent.futures import Future
            f = Future()
            f.set_result(svg_file)
            futures.append(f)
            continue
        future = _tex_executor.submit(
            _compile_and_convert_sync, tex_file, tex_template
        )
