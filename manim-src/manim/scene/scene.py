...
class Scene:
    ...
    def exec_code(self, code: str, env: dict | None = None) -> dict:
        """在持久化模式下执行代码字符串

        exec_env 包含 self（Scene 实例）和 manim 全部 API，
        因此 'c = Circle(color=RED)' 和 'self.play(Write(c))' 天然可用。
        ...
        code : str
            要执行的 Python 代码
        env : dict, optional
            额外环境变量，会合并到 exec_env

        Returns
        -------
        dict : {"success": bool, "new_vars": list, "error": str|None}
        """
        if not hasattr(self, '_persistent_env') or self._persistent_env is None:
            self._init_persistent_env()
        e = dict(self._persistent_env)
        if env:
            e.update(env)
        known = set(e.keys())
        try:
            exec(code, e)
            from manim.mobject.mobject import Mobject
            new_vars = [k for k in e if k not in known and not k.startswith('_')
                        and isinstance(e[k], (Mobject,))]
            self._persistent_env.update({k: e[k] for k in new_vars})
            for k in new_vars:
                mob = e[k]
                if mob not in self.mobjects:
                    self.add(mob)
            return {"success": True, "new_vars": new_vars, "error": None}
        except Exception as exc:
            return {"success": False, "new_vars": [], "error": str(exc)}

    def _init_persistent_env(self):
        """初始化持久化 exec 环境——self + manim 全部 API"""
        import manim as _manim
        self._persistent_env = {"self": self, "scene": self}
        self._persistent_env.update({k: v for k, v in vars(_manim).items() if not k.startswith('_')})

    def get_frame_jpeg(self, quality: int = 75) -> bytes:
        """获取当前帧的 JPEG 字节数据"""
        import numpy as np
